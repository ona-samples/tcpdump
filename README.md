...
This repository configures an Ona devcontainer for capturing network traffic and
running simple test endpoints. The devcontainer runs in privileged mode with
host networking and includes `tcpdump`, `tshark`, `capinfos`, `termshark`, and
VS Code pcap viewing extensions.
...
The service captures traffic on the known VM SSH endpoint ports and the test
endpoint ports:
